...
Discord の新規投稿を受け取り、俳句・川柳・それに近い短詩だけを AI で検出する。対象作品には、具体的な表現に触れた親しみやすい講評、5 項目の 5 段階評価、総合評価、和風の作品画像を返信する。
...
- 二次生成の 1 回の Responses API リクエストで、構造化された講評と画像生成ツールの結果を取得する。
- 画像生成 AI に日本語本文を描かせない。生成された背景バイト列に、Bot が Pillow で元の作品本文を合成する。
...
- `ratings` の 5 項目と `overall` は 1 から 5 の整数。
...
3. 対象なら、二次生成を Responses API の構造化出力と `image_generation` 組み込みツールの同一リクエストで実行する。
4. 画像生成ツールは正方形、和紙、控えめな木製額縁、水墨画風、余白、淡い情景、抽象的な赤い落款、可読文字なしを指示する。
5. Responses の `output` から `image_generation_call.result` を Base64 デコードし、講評モデルと同じ作品コンテキストで保持する。

SDK は `AsyncOpenAI` を使い、実行時のモデル名は環境変数で差し替え可能にする。実装時点では Responses API の `responses.parse(..., text_format=...)` と `tools=[{"type": "image_generation", ...}]` を前提とする。

## 5. Discord 投稿契約

対象投稿への返信本文は次の順序で組み立てる。

1. `作品種別: 俳句` または `作品種別: 川柳`
2. 元の作品本文（Discord に投稿された本文をそのまま表示）
3. 講評
4. `情景`、`余韻`、`独創性`、`言葉選び`、`ユーモア・風刺` の星評価
5. 総合評価
...
- AI サービス: 一次判定が対象外なら二次呼び出しを行わない、対象なら二次を一度だけ呼ぶ、画像結果の抽出。
- Discord ハンドラ: Bot 投稿・重複投稿の無視、対象外の無返信、対象作品の reply。

## 10. 実装前に確認する事項

以下は実装の品質・費用・障害時の見え方に影響するため、確定後に既定値として仕様へ反映する。

- 一次判定と二次生成の既定モデル。
- 画像生成結果が得られない場合に、講評だけを返信するか、全体を返信しないか。
- 空本文・画像だけの Discord 投稿を一次判定へ送るか、入力なしとして無視するか。
