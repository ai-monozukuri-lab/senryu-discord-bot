...
- 背景テンプレートに日本語本文は含めず、Bot が Pillow で `extracted_text` を合成する。
...
4. 画像はリポジトリの固定テンプレートから読み込み、Pillow で `extracted_text` を合成する。画像生成 API は使用しない。
...
- `extracted_text` を行単位で保持し、縦書き列へ描く。文字列の省略や AI による正規化は行わない。
...
- 画像合成: 正方形 PNG、`extracted_text` の描画、フォント未発見時の明示的エラー。
